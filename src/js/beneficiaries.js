...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.to == "ben") {
    ...
    if (request.order === "start" && token_benef == null) {
      ...
      isSelectRewardDropdownEnabled =
        request.data.select_reward_dropdown_enabled;
...
function startBeneficiaries() {
  ...
  if (
    regexCreatePostSteemit.test(window.location.href) ||
    regexCreatePostBusy.test(window.location.href)
  )
...
function addBeneficiariesButton() {
  ...
  if (isSteemit) {
    ...
    $(".vframe__section--shrink")[
      $(".vframe__section--shrink").length - 1
    ].after(benef_div);
    ...
    $(".benef").click(function() {
      ...
      $(".benef")
        .parent()
        .after(
          '<li class="beneficiaries"><div class="benef_elt"><span class="sign" >@</span><input type="text" placeholder="username"></div><div class="benef_elt" style="width: 15%;"><input style="width: 75%;" type="number" placeholder="10"><span class="sign" >%</span></div><a  class="close"></a> </li>'
        );
      ...
      if (!isPremiumBeneficiaries)
        $(".benef")
          .parent()
          .after(
            '<p class="message-beneficiaries">By using the beneficiaries feature, you accept that @steem-plus will be set as a 5% beneficiary.</p>'
          );
      else
        $(".benef")
          .parent()
          .after(
            '<p class="message-beneficiaries">You subscribe to the beneficiaries premium feature. You won\'t be charged 5%.</p>'
          );
      ...
      if ($(".close").length === 1) {
        ...
        if (isSelectRewardDropdownEnabled) {
          ...
          if ($(".post").length === 0) {
            $(".beneficiaries").after(
              '<li class="post"><div class="inline_button"><input type="button" class="UserWallet__buysp button postbutton" value="Post"/></div></li>'
            );
            $(".postbutton").click(function() {
              if (isEverythingFilled()) postBeneficiaries();
            });
      ...
      if ($(".benef-steemit-percentage").length === 0) {
        $(".benef")
          .parent()
          .before(
            '<div class="div-benef-steemit-percentage"><label>Reward</label><select class="benef-steemit-percentage ant-form-item-control has-success">\
  ...
  // if busy
    ...
    $(".benef").click(function() {
      ...
      if ($(".benef-busy-percentage").length === 0) {
        $(".ant-form-item-control").each(function() {
          if (
            $(this)
              .children()
              .find(".ant-select-selection-selected-value").length > 0
          ) {
            // ... create it
            $(this).after(
              '<select class="benef-busy-percentage ant-form-item-control has-success">\
                              <option name="percentage" value="10000">50% SBD and 50% SP</option>\
                              <option name="percentage" value="0">100% Steem Power</option>\
                              <option name="percentage" value="-1">Decline Payout</option>\
                            </select>'
      ...
      $(".benef")
        .parent()
        .after(
          '<li class="beneficiaries"><div class="benef_elt benef_elt_busy"><span class="sign" >@</span><input type="text" placeholder="username"></div><div class="benef_elt benef_elt_busy" style="width: 15%;"><input style="width: 75%;" type="number" placeholder="10"><span class="sign" >%</span></div><a  class="close"></a> </li>'
        );
      ...
      if ($(".close").length === 1) {
        ...
        if ($(".post").length === 0) {
          $(".beneficiaries").after(
            '<li class="post"><div><input type="button" class="Action postbutton-busy Action--primary" value="Post"/></div></li>'
          );
    ...
    $(this).on("click", function() {
      ...
      if ($(".close").length === 0) {
        ...
        if (isSteemit) {
          ...
          if (isSelectRewardDropdownEnabled) $(".postbutton").hide();
          else {
            $("h5,.post").hide();
...
function isEverythingFilled() {
  ...
  if (isSteemit) {
    ...
    if (
      $(".Editor__title")
        .eq(0)
        .val() === "" ||
      $(".ant-select-selection__choice__content").length === 0 ||
      $("textarea#body").eq(0).innerHTML === ""
    ) {
      alert("Please enter a title, body and tags to your post!");
      return false;
    }
  ...
  if (hasWrongPercent !== 0) {
    alert(
      "Percentage should be greater than 0 and smaller or equal to 100! (Including 5% SteemPlus fee)"
    );
    return false;
  }
  if (total_percent > 95) {
...
async function postBeneficiaries() {
  ...
  if (isSteemit) {
    ...
    body = $(".vframe textarea")
      .eq(0)
      .val();
    sbd_percent = $(".benef-steemit-percentage")
      .eq(0)
      .val();
  } else if (isBusy) {
    ...
    body = $("textarea.ant-input")
      .eq(0)
      .val();
    sbd_percent = $(".benef-busy-percentage")
      .eq(0)
      .val();
  }

  if (
    isPremiumBeneficiaries ||
    beneficiaries.filter(elt => {
      return elt.account == "steemplus-pay";
    }).length == 1
  )
    console.log("no fee");
  else
  ...
  if (beneficiaries.length > 6) {
    alert(
      "You have set up too many beneficiaries (max number=5, 6 for premium users)"
    );
  }
  ...
  if (connect.method == "sc2") {
    api.broadcast(operations, function(e, r) {
      if (e) {
        console.log(e);
        if (e.error !== undefined) {
          ...
          steem.api.getAccounts(usernamesBenef, function(err, result) {
            ...
            if (errorUsernames.length > 0)
              alert(
                "The following usernames are not correct : " +
                  errorUsernames.join(", ")
              );
            // If all the accounts name are correct then display error message. Problem might come from steemConnect.
            else
              alert(
                "The request was not succesfull. Please make sure that you logged in to SteemPlus via SteemConnect, that all the beneficiaries accounts are correct and than you didn't post within the last 5 minutes. If the problem persists please contact @stoodkev on Discord. Error description:" +
                  e.error_description
              );
          });
        }
      } else {
        if (isSteemit) window.location.replace("https://steemit.com");
        if (isBusy) window.location.replace("https://busy.org");
      }
    });
  } else {
    steem_keychain.requestBroadcast(
      ...
      function(result) {
        ...
        if (result.success) {
          ...
          alert(
            "The request was not succesfull. Please make sure that you logged in to SteemPlus via an account having Posting authority on Keychain, that all the beneficiaries accounts are correct and than you didn't post within the last 5 minutes. If the problem persists please contact @stoodkev on Discord. Error description:" +
