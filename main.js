  ...
  if(steemConnect.connect===true&&steemConnect.tokenExpire>Date.now()){
    ...
    sc2.me().then((me)=>
    {
      ...
      if(beneficiaries&&(steemit||busy))
        chrome.runtime.sendMessage({ token:token, to: 'ben', order: 'start',data:{user:user, steemit:steemit, busy:busy, select_reward_dropdown_enabled:true}});
      ...
      chrome.runtime.sendMessage({ token:token, to: 'ben_d', order: 'start',data:{user:user, steemit:steemit, busy:busy, select_reward_dropdown_enabled:true}});
      ...
      $(document).click(function(){
        setTimeout(function(){
          if(urlOnline!==window.location.href)
          {
            ...
            if(beneficiaries&&(steemit||busy))
              chrome.runtime.sendMessage({ token:token, to: 'ben', order: 'click',data:{user:user, steemit:steemit, busy:busy, select_reward_dropdown_enabled:true}});
            ...
            chrome.runtime.sendMessage({ token:token, to: 'ben_d', order: 'click',data:{user:user, steemit:steemit, busy:busy, select_reward_dropdown_enabled:true}});
...
function startOfflineFeatures(items, user, account)
{
  ...
  if(witnesses_tab&&(steemit||busy))
      chrome.runtime.sendMessage({ token:token, to: 'witnesses_tab', order: 'start',data:{user:user, account:account,totalSteem:totalSteemLS,totalVests:totalVestsLS, steemit:steemit, busy:busy}});
  ...
  if(tip_user&&(steemit||busy))
      chrome.runtime.sendMessage({ token:token, to: 'tip_user', order: 'start',data:{user:user, steemit:steemit, busy:busy}});
  ...
  if (steemit_more_info) {
    ...
    if((steemit||busy)&&vote_tab)
      chrome.runtime.sendMessage({ token:token, to: 'vote_tab', order: 'start',data:{steemit:steemit, busy:busy, rewardBalance:rewardBalanceLS, recentClaims:recentClaimsLS, steemPrice:steemPriceLS}});
    ...
    if((steemit||busy)&&post_floating_bottom_bar)
      chrome.runtime.sendMessage({ token:token, to: 'post_floating_bottom_bar', order: 'start',data:{steemit:steemit, busy:busy}});
  }
  ...
  $(document).click(function(){
    ...
    setTimeout(function(){
      if(urlOffline!==window.location.href)
      {
        ...
        if(classification_user&&(steemit||busy))
          chrome.runtime.sendMessage({ token:token, to: 'classification_user', order: 'click',data:{user:user, steemit:steemit, busy:busy}});
        if(witnesses_tab&&(steemit||busy))
          chrome.runtime.sendMessage({ token:token, to: 'witnesses_tab', order: 'click',data:{user:user,account:account,totalSteem:totalSteemLS,totalVests:totalVestsLS, steemit:steemit, busy:busy}});
        if(article_count&&(steemit||busy))
        ...
        if(author_popup_info&&(steemit||busy))
          chrome.runtime.sendMessage({ token:token, to: 'author_popup_info', order: 'click',data:{user:user, steemit:steemit, busy:busy}});
        if(add_signature&&(steemit||busy))
