...
function startClassificationUser() {
    ...
    if (elementUserListCU !== null)
        elementUserListCU.each(function(indexItem, item) {
            if (!$(item).hasClass('has-classification')) {
                ...
                if (userListCU.find(function(e) {
                        return e.username === usernameCurrentItem
                    }) === undefined)
                    userListCU.push({
                        username: usernameCurrentItem,
                        arrayElement: [item],
                        userScoreList: []
                    });
                else {
                    userListCU.find(function(e) {
                        return e.username === usernameCurrentItem
    ...
    if (userListCU.length > 0) {
        var arrayUsernames = userListCU.map(function(e) {
            return e.username;
        });
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                Object.keys(result.data).map(function(objectKey, index) {
                    ...
                    userListCU.find(function(e) {
                        return e.username === objectKey
                    }).userScoreList = userScoreList.slice();
...
function sendRequestAPICU(classificationParam, usernameParam, permlinkParam) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            "reporting_account": myUsernameCU,
