...
function waitForUI() {
  console.log("wait");
  setTimeout(function() {
    if ($(".card-img").length != 0) startBatchPurchase();
    else {
      waitForUI();
    }
  }, 250);
...
function startBatchPurchase() {
  ...
  batchButton.title =
    "Get free SteemPlus Points while buying Steem Monsters cards for the same price.";
  $(".buttons").append(batchButton);

  $(".buttons").append($("<span id='total'></span>"));
  $(".filter-form .vertical-center").append(
  ...
  $("#input_sm").on("input", function() {
    ...
    if ($(".select-sm select option:selected").val() == "Cheapest") {
      ...
      const nbCards = Math.min(
        parseInt($("#input_sm").val()),
        $(".card-checkbox:not(:hidden)").length
      );
      for (let i = 0; i < nbCards; i++) {
        $(".card-checkbox:not(:hidden)")
          .eq(i)
          .click();
      }
  ...
  $("#batch_buy").click(function() {
    if ($(".checked").length == 0) alert("Please choose some cards first");
    total_price = 0;
    // Store list of cards to be purchased
    for (card of $(".checked")) {
      batch.push({
        market_id: $(card).attr("market_id"),
        price: $(card).attr("price")
      });
...
async function sendTransfer(total_price) {
  ...
  if (total_rate.includes("DEC")) {
    const cards = batch.map(e => e.market_id);
    const obj = {items: cards, purchaser: username, market: "steemplus-pay"};
    await steem_keychain.requestCustomJson(
      username,
      "sm_market_purchase",
      "active",
      JSON.stringify(obj),
    ...
    const steemconnect_sign =
      "https://steemconnect.com/sign/transfer?from=" +
      username +
      "&to=steemmonsters&amount=" +
      total_rate +
      "%20" +
      $("#payment_currency option:selected").val() +
      "&memo=" +
      memo;
    if (hasSKC) {
      ...
      steem_keychain.requestTransfer(
        ...
        $("#payment_currency option:selected").val(),
        function(result) {
          if (!result.success) window.open(steemconnect_sign);
        },
        true
      );
    } // Request payment via Steemconnect
    else window.open(steemconnect_sign);
  }
}
...
async function getPrice(total_sm) {
  ...
  console.log($("#payment_currency option:selected").val());
  switch ($("#payment_currency option:selected").val()) {
    case "STEEM":
      return (total_sm / market.steem_price).toFixed(3);
    case "SBD":
      return (total_sm / market.sbd_price).toFixed(3);
    default:
      console.log(total_sm, market.dec_price, market);
      return (total_sm / market.dec_price).toFixed(3) + " DEC";
  }
}
