                ...
                if (followers_table)
                    chrome.runtime.sendMessage({
                        ...
                        data: {
                            ...
                            votePowerReserveRate: votePowerReserveRateLS,
                            account: account,
                            totalVestingFund: totalSteemLS,
                            totalVestingShares: totalVestsLS
                        }
                    });
            }
            ...
            if (busy && steemit_more_info) {
                if (vote_weight_slider_busy)
                    chrome.runtime.sendMessage({
                        ...
                        order: 'start',
                        data: {
                            isPostFloatingBottomBarEnabled: isPostFloatingBottomBarEnabled,
                            rewardBalance: rewardBalanceLS,
                            recentClaims: recentClaimsLS,
                            steemPrice: steemPriceLS,
                            votePowerReserveRate: votePowerReserveRateLS,
...
function startOfflineFeatures(items, user, account) {
    ...
    const steem_monsters = (items.steem_monsters == undefined || items.steem_monsters == 'show');
    ...
    if (wallet_history && steemit)
        chrome.runtime.sendMessage({
            ...
            data: {
                totalSteem: totalSteemLS,
                totalVests: totalVestsLS,
                walletHistoryMemoKey: wallet_history_memo_key,
                account: account
            }
        });
        ...
        if ((steemit || busy) && boost_button)
            chrome.runtime.sendMessage({
                ...
                data: {
                    ...
                    steemit: steemit,
                    busy: busy,
                    post_floating_bar: post_floating_bottom_bar,
                    global: {
                        rewardBalance: rewardBalanceLS,
                        recentClaims: recentClaimsLS,
                        steemPrice: steemPriceLS,
